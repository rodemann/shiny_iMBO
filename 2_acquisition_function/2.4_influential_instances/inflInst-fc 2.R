...
# TO DO:
# -only works for built in Infill Crits
# -ggf. flip the sign of infl
...
# -ggf. (eher nicht) predict also the values of the other seen point in the iter
# -ic arguments like se.threshold as arguments of the function, otherwise not possible to change
# -interest = c("prediction", "af", "target")
...
inflInst <- function(res.mbo, iter, influence =  "normal", ...#seed = NULL
                     ) {
  ...
  result.without = lapply(1:nrow(new.design), function(to.remove.index) {
    ...
    result.mbo.hypo = list(
     ...
     # proposed points, target and value of the of iter i, dob > 0 is fine because only 1 iter is run
...
A = inflInst(res.mbo = res.mbo, iter = 5, influence = "normal")
