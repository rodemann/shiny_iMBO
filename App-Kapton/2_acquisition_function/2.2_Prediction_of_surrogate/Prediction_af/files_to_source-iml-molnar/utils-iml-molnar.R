...
get.grid <- function(dat, grid.size, anchor.value = NULL, type = "equidist") {
  checkmate::assert_data_frame(dat, min.cols = 1)
  features <- colnames(dat)
  feature.type <- unlist(lapply(dat, function(x) {
    get.feature.type(class(x))
  }))
  checkmate::assert_character(features, min.len = 1, max.len = 2)
  checkmate::assert_true(length(features) == length(feature.type))
  checkmate::assert_numeric(grid.size, min.len = 1, max.len = length(features))
...
order_levels <- function(dat, feature.name) {
  checkmate::assert_data_frame(dat)
  checkmate::assert_character(feature.name)
  checkmate::assert_true(feature.name %in% names(dat))
  checkmate::assert_factor(dat[, feature.name, with = FALSE][[1]])
...
get_layout <- function(n_features, nrows = NULL, ncols = NULL) {
  checkmate::assert_integerish(n_features, lower = 1, null.ok = FALSE, any.missing = FALSE)
  checkmate::assert_integerish(ncols, lower = 1, null.ok = TRUE, len = 1, any.missing = FALSE)
  checkmate::assert_integerish(nrows, lower = 1, null.ok = TRUE, len = 1, all.missing = FALSE)
